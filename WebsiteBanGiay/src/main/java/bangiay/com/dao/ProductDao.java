...
public interface ProductDao extends JpaRepository<Product, Integer> {
	...
	@Query(value = "SELECT p.ID, p.NAME, (SELECT SUM(od.QUANTITY) FROM order_detail od, orders o WHERE o.status = 4 AND o.returnStatus != 5 AND od.ORDER_ID = o.id and od.SIZE_ID = s.ID) as ORDER_QUANTITY, (SELECT SUM(od.QUANTITY*od.PRICE) FROM order_detail od, orders o WHERE o.status = 4 AND o.returnStatus != 5 AND od.ORDER_ID = o.id and od.SIZE_ID = s.ID) as TOTAL, (SELECT url FROM media WHERE PRODUCT_ID = p.ID LIMIT 1) as image FROM product p LEFT JOIN size s ON p.ID = s.PRODUCT_ID LEFT JOIN ("
			...
			+ ") as d ON s.ID = d.SIZE_ID" + " ORDER BY TOTAL DESC", nativeQuery = true)
