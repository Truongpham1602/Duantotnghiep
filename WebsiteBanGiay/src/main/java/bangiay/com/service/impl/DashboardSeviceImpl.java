...
import bangiay.com.DTO.CommonValueDTO;
import bangiay.com.DTO.DashboardCountDTO;
import bangiay.com.DTO.DashboardDTO;
import bangiay.com.DTO.DashboardOrderDetailDTO;
import bangiay.com.DTO.DashboardRevenueDTO;
import bangiay.com.dao.DashboardDao;
...
public class DashboardSeviceImpl implements DashboardService {
	...
	public void getRevenue(DashboardDTO res) {
		...
		for (int i = 6; i >= 0; i--) {
			...
			Integer month = currentDate.getMonth() - i >= 0 ? currentDate.getMonth() - i
					: 12 + currentDate.getMonth() - i;
		...
		for (Map<String, Object> item : listOrderDetail) {
			...
			// Count money 1 day
			if (itemDate.getTime() >= setTimeNewDay(new Date()).getTime()
					&& itemDate.getTime() <= setTimeLastDay(new Date()).getTime()) {
				dayMoney += (Integer) item.get("quantity") * (Double) item.get("price");
			}
			if (itemDate.getTime() >= setTimeNewDay(new Date(newDate.getTime() - 86400000)).getTime()
					&& itemDate.getTime() <= setTimeLastDay(new Date(newDate.getTime() - 86400000)).getTime()) {
				lastDayMoney += (Integer) item.get("quantity") * (Double) item.get("price");
			}

			// Count money 1 week
			if (itemDate.getTime() >= setTimeNewDay(new Date(newDate.getTime() - 7 * 86400000)).getTime()
					&& itemDate.getTime() <= setTimeLastDay(new Date()).getTime()) {
				weekMoney += (Integer) item.get("quantity") * (Double) item.get("price");
			}
			if (itemDate.getTime() >= setTimeNewDay(new Date(newDate.getTime() - 14 * 86400000)).getTime()
					&& itemDate.getTime() <= setTimeLastDay(new Date(newDate.getTime() - 7 * 86400000)).getTime()) {
				lastWeekMoney += (Integer) item.get("quantity") * (Double) item.get("price");
			}

			// Count money 1 month
			if (itemDate.getTime() >= setTimeNewDay(new Date((newDate.getTime() - (long) 30 * 86400000))).getTime()
					&& itemDate.getTime() <= setTimeLastDay(new Date()).getTime()) {
				monthMoney += (Integer) item.get("quantity") * (Double) item.get("price");
			}
			if (itemDate.getTime() >= setTimeNewDay(new Date(newDate.getTime() - (long) 60 * 86400000)).getTime()
					&& itemDate.getTime() <= setTimeLastDay(new Date(newDate.getTime() - (long) 30 * 86400000))
							.getTime()) {
				lastMonthMoney += (Integer) item.get("quantity") * (Double) item.get("price");
			}

			// Count money 1 year
			if (itemDate.getTime() >= setTimeNewDay(new Date((newDate.getTime() - (long) 365 * 86400000))).getTime()
					&& itemDate.getTime() <= setTimeLastDay(new Date()).getTime()) {
				yearMoney += (Integer) item.get("quantity") * (Double) item.get("price");
			}
			if (itemDate.getTime() >= setTimeNewDay(new Date(newDate.getTime() - (long) 730 * 86400000)).getTime()
					&& itemDate.getTime() <= setTimeLastDay(new Date(newDate.getTime() - (long) 365 * 86400000))
							.getTime()) {
			...
			if (mapChartRevenueByDay.containsKey(keyEvenueByDay)) {
				mapChartRevenueByDay.put(keyEvenueByDay, (long) (mapChartRevenueByDay.get(keyEvenueByDay)
						+ (Integer) item.get("quantity") * (Double) item.get("price")));
			}

			String keyEvenueByMonth = (itemDate.getMonth() + 1) + "/" + (itemDate.getYear() + 1900);
			if (mapChartRevenueByMonth.containsKey(keyEvenueByMonth)) {
				mapChartRevenueByMonth.put(keyEvenueByMonth, (long) (mapChartRevenueByMonth.get(keyEvenueByMonth)
						+ (Integer) item.get("quantity") * (Double) item.get("price")));
			}
		}

		res.getRevenue()
				.add(new DashboardRevenueDTO(dayMoney, lastDayMoney == 0 ? 100
						: (int) (dayMoney == 0 ? -100
								: Math.floor(((dayMoney > lastDayMoney ? 0 : -1) + dayMoney / lastDayMoney) * 100))));
		res.getRevenue().add(new DashboardRevenueDTO(weekMoney, lastWeekMoney == 0 ? 100
				: (int) (weekMoney == 0 ? -100
						: Math.floor(((weekMoney > lastWeekMoney ? 0 : -1) + weekMoney / lastWeekMoney) * 100))));
		res.getRevenue().add(new DashboardRevenueDTO(monthMoney, lastMonthMoney == 0 ? 100
				: (int) (monthMoney == 0 ? -100
						: Math.floor(((monthMoney > lastMonthMoney ? 0 : -1) + monthMoney / lastMonthMoney) * 100))));
		res.getRevenue().add(new DashboardRevenueDTO(yearMoney, lastYearMoney == 0 ? 100
				: (int) (yearMoney == 0 ? -100
						: Math.floor(((yearMoney > lastYearMoney ? 0 : -1) + yearMoney / lastYearMoney) * 100))));
		...
		for (int i = 0; i < dataChartRevenueByMonth.size(); i++) {
			dataChartRevenueByMonth.get(i)
					.setValue(mapChartRevenueByMonth.get(dataChartRevenueByMonth.get(i).getKey()));
		}
	...
	public void getDataChartOrder(DashboardDTO res) {
		...
		Date newDate = new Date();
		List<Order> listOrderOneWeek = orderDao
				.getOrderOneWeek(setTimeNewDay(new Date(newDate.getTime() - 7 * 86400000)));
	...
	public void getListProduct(DashboardDTO res) {
		...
		for (Map<String, Object> item : listProductDetail) {
			...
			if (mapProduct.containsKey(key)) {
				...
				itemMap.setQuantity(itemMap.getQuantity()
						+ (item.get("order_quantity") != null ? ((BigDecimal) item.get("order_quantity")).intValue()
								: 0));
				itemMap.setRevenue(
						itemMap.getRevenue() + (item.get("total") != null ? ((Double) item.get("total")) : 0));
				...
				mapProduct.put((Integer) item.get("id"),
						new DashboardOrderDetailDTO((Integer) item.get("id"), (String) item.get("name"),
								((BigDecimal) item.get("order_quantity")) != null
										...
										: 0,
								item.get("total") != null ? (Double) item.get("total") : 0,
